			...
			buttons = [
				{
					...
					onclick: ev => Theatre.onAddToNavBar(ev,this)
				}
			].concat(buttons); 
		}
...
Hooks.on("preCreateChatMessage", function(context,chatData) {
	...
	if (!chatData.roll && Theatre.instance.speakingAs && Theatre.instance.usersTyping[chatData.user]) {
 ...
 */
Hooks.on("createChatMessage", function(chatEntity) {
	...
	if (chatData.content.startsWith('/') 
	|| chatData.roll
	|| chatData.emote
	|| chatData.type == 1
	...
	if (textBox) {
		...
		if (insert) {
			...
			Theatre.instance.portraitDocks.sort((a,b)=>{return a.renderOrder-b.renderOrder}); 
			...
			tween = TweenMax.to(insert.portrait,0.25,{
				//pixi:{tint: 0xAAEDFF},
				pixi:{tint: Theatre.instance.getPlayerFlashColor(userId,insert.textColor)},
				ease: Power3.easeOut,
				repeat: 1,
				yoyo: true,
				onComplete: function(ctx,imgId,tweenId) {
		...
		Theatre.textFlyinAnimation(insertFlyinMode || "typewriter")
			.call(this,charSpans,0.5,0.05,
		...
		if (insert && Theatre.instance.settings.autoDecay) {
			insert.decayTOId = window.setTimeout((imgId)=>{
				let insert = Theatre.instance.getInsertById(imgId); 
				if (insert)
					Theatre.instance.decayTextBoxById(imgId,true); 
			},Math.max(Theatre.instance.settings.decayRate*charSpans.length,Theatre.instance.settings.decayMin),insert.imgId); 
		}
	}
}); 
...
Hooks.on("renderChatLog",  function() {
	...
	console.log("%cTheatre is Ready!","font-weight: bold; font-size: 40px; font-style: italic; color: cyan; text-shadow: 1px 1px 0px black,  1px -1px 0px black, -1px 1px 0px black, -1px -1px 0px black;"); 
	...
	console.log(tux); 
	console.log("%c-- Theatre is Powered by Free Open Source GPLv3 Software --","font-weight: bold; font-size: 18"); 
